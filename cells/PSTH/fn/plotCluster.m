function plotCluster (Unit,PSTH, PSTHAdaptive, Param, Spikes, trials_in_session)
...
title(sprintf('anm%d    %s     Unit #%d      Quality: %s    Cell-type: %s \nShank: %d      site: %d     depth: %d um    mean FR %.2f (Hz)    peak FR %.2f (Hz)',Unit.subject_id, Unit.session_date, Unit.unit_uid, Unit.unit_quality, Unit.cell_type, Unit.electrode_group, Unit.unit_channel, Unit.unit_dv_location, Unit.mean_fr, Unit.peak_fr_basic_trials),'FontSize',12,'HorizontalAlignment','left');   
...
% fn_plotSimpleRaster(Param, Spikes,trials_in_session)
