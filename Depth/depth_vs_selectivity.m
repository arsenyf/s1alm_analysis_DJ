...
unit_selectivity = movmean(cell2mat(fetchn(rel_Selectivity,'unit_selectivity','ORDER BY unit_uid')) ,[smooth_bins 0], 2, 'Endpoints','shrink');
...
xlim([min(depth),max(depth)]);
 ...
 plot(t_max(idx_right_selective),depth(idx_right_selective),'.')
 xlabel('Time of max selectivity (s)');
 ylabel(' Depth (um)');
 ylim([min(depth(idx_right_selective)),max(depth(idx_right_selective))]);
...
xlim([min(depth(idx_right_selective)),max(depth(idx_right_selective))]);
 ...
 plot(t_max(idx_left_selective),depth(idx_left_selective),'.')
 xlabel('Time of max selectivity (s)');
 ylabel(' Depth (um)');
 ylim([min(depth(idx_left_selective)),max(depth(idx_left_selective))]);
