...
xlabel('Time (s)');
ylabel('Performance (%)');
set(gca, 'Xtick', [0.5, (-3.8+disp_offset),(-2.5+disp_offset), (-1.6+disp_offset), (-0.8+disp_offset),disp_offset], 'XtickLabel', {'Control', '-3.8','-2.5','-1.6', '-0.8','0',}, 'Ytick',[0,25,50,75,100], 'TickLabelInterpreter', 'None', 'FontSize', 11, 'XTickLabelRotation', 0);
...
plot([0+disp_offset 0+disp_offset],[0 100],'-k');


for i=1:1:numel(trn_r)
    text(x_r(i)-0.025,y_r(i)-10,behav_param_signif(trn_r(i)),'HorizontalAlignment','Center','FontSize',16,'FontWeight','bold','Color',[0 0 1])
end

for i=1:1:numel(trn_l)
    text(x_l(i)+0.025,y_l(i)+7,behav_param_signif(trn_l(i)),'HorizontalAlignment','Center','FontSize',16,'FontWeight','bold','Color',[1 0 0])
end
xlim([0 disp_offset+0.1]);
