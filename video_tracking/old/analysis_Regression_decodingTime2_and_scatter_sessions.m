function analysis_Regression_decodingTime2_and_scatter_sessions()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\regression_sessions_all3\' 't' num2str(key.regression_time_start) '\'];
...
rel=ANL.RegressionProjTrialNormalizedSignif3 *ANL.SessionPosition *EXP.SessionTraining;
...
for i_p=1:1:numel(tuning_param_name_1D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        for i_s=1:1:numel(sessions)
            ...
            num_signif_cells(i_s) = sum((fetchn((rel_significant_cells&rel)*EXP.SessionID & key, 'flag_regression_significant')));
            subplot(6,5,i_s);
            [R2_LinearRegression(i_s),R2_LogisticRegression(i_s)]=fn_plot_regression_scatter_session (rel,rel_P,  key, tuning_param_label{i_p});
        end
