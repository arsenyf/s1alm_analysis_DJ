function videoTongue_neuralActivitySpaceRegressionZSign()
close all;
...
session_uid=unique(fetchn( ((EXP.SessionID*EXP.SessionTraining*ANL.SessionPosition) & ANL.Video1stLickTrialZscore  & k & ANL.IncludeSession)-ANL.ExcludeSession,'session_uid'));
...
%     k_s.session_uid=session_uid(i_s);
        k_s = k;
    rel_behav= EXP.TrialID &((EXP.BehaviorTrial * EXP.SessionID * EXP.SessionTraining *ANL.SessionPosition * EXP.TrialName  * ANL.TrialTypeGraphic * ANL.LickDirectionTrial) & ANL.Video1stLickTrialZscore   & k  & 'early_lick="no early"' & k_s & ANL.IncludeSession)-ANL.ExcludeSession;
    TONGUE = struct2table(fetch((ANL.Video1stLickTrialZscore & rel_behav)*EXP.TrialID,'*' , 'ORDER BY trial_uid'));
    ...
    rel_Proj = ((ANL.RegressionProjTrialNormalized2 & rel_behav) &k_proj  )*EXP.TrialID*EXP.TrialName;
