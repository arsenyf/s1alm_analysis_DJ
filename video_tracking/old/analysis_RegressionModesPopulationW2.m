function analysis_RegressionModesPopulationW2()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\RegressionModes_population2_w2\' 't' num2str(key.regression_time_start) '\'];
...
rel=ANL.RegressionDecoding2Weigts2*ANL.SessionPosition*EXP.SessionTraining;
rel_P=ANL.RegressionProjTrial2Weights2*ANL.SessionPosition*EXP.SessionTraining;
...
for i_p=1:1:numel(tuning_param_name_1D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        fn_plot_regression_scatter_population_logistic_vs_linear (rel_P, key, tuning_param_label{i_p})
