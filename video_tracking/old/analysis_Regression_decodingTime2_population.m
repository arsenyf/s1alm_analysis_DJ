function analysis_Regression_decodingTime2_population()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\regression_decoding2\' 't' num2str(key.regression_time_start) '\'];
...
rel=ANL.RegressionDecodingNormalized2Time*ANL.SessionPosition*EXP.SessionTraining;
rel_P=ANL.RegressionProjTrialNormalizedSignif2*ANL.SessionPosition*EXP.SessionTraining;
...
for i_p=1:1:numel(tuning_param_name_1D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        rel_significant_cells=ANL.RegressionUnitSignificanceRsq2& key;

        axes('position',[position_x1(i_p), position_y1(i_d), panel_width1, panel_height1]);
        fn_plot_regression_decoding_time (rel,  key, tuning_param_label{i_p}, rel_significant_cells);
        
        axes('position',[position_x1(i_p)+0.13, position_y1(i_d), panel_width1*1.5, panel_height1]);
        fn_plot_regression_scatter_population (rel,rel_P,  key, tuning_param_label{i_p})
