function analysis_RegressionModes2DPopulationGo()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\RegressionModes2D_population2Go\' 't' num2str(key.regression_time_start) '\'];
...
tuning_param_name_2D{1}.x='lick_horizoffset';
tuning_param_name_2D{2}.x='lick_horizoffset';
...
for i_p=1:1:numel(tuning_param_name_2D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        fn_plot_regression_2DGo (rel_P, key, tuning_param_name_2D{i_p}, tuning_param_label_2D{i_p}, graphic_param)
