function analysis_RegressionModesPopulationGo()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\RegressionModes_populationGo_signif\' 't' num2str(key.regression_time_start) '\'];
...
rel=ANL.RegressionDecoding*ANL.SessionPosition*EXP.SessionTraining;
rel_P=ANL.RegressionProjTrialGo*ANL.SessionPosition*EXP.SessionTraining & ANL.VideoTongueValidRTTrial;
...
for i_p=1:1:numel(tuning_param_name_1D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        fn_plot_regression_decoding_time_logistic_vs_linearGo (rel,  key, tuning_param_label{i_p});
        
        axes('position',[position_x1(i_p)+0.13, position_y1(i_d), panel_width1*1.5, panel_height1]);
        fn_plot_regression_scatter_population_logistic_vs_linearGo (rel_P, key, tuning_param_label{i_p})
