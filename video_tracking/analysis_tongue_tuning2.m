...
key_2D.tuning_param_name_x='lick_yaw_relative';
key_2D.tuning_param_name_y='lick_peak_x';
...
key2.outcome='all';
key2.smooth_flag=1;
key2.tuning_param_name='lick_yaw_relative';
key2.time_window_start=-0.4;
rel1= (ANL.UnitTongue1DTuning*ANL.UnitTongue1DTuningSignificance & key2  & 'number_of_trials>100' & 'number_of_spikes_window>100' & 'pvalue_si_1d<=0.01' )  & (EPHYS.UnitCellType*EPHYS.UnitPosition & key1 ) ;
key2.time_window_start=0;
rel2=(ANL.UnitTongue1DTuning*ANL.UnitTongue1DTuningSignificance & key2  & 'number_of_trials>100' & 'number_of_spikes_window>100' & 'pvalue_si_1d<=0.01')  & (EPHYS.UnitCellType*EPHYS.UnitPosition & key1 ) ;
...
tuning_param_name{1}='lick_yaw_relative';
...
histogram(TUNING{2}.XY.stability_odd_even_corr_r)
histogram(TUNING{2}.XY.tongue_tuning_2d_si)
...
for i_u=1:1:size(TUNING{1}.X,1)
    ...
    xlabel(vname);
    ylabel('FR (Hz)');
    title(sprintf('uid= %d %s %s %s \n t wind=[%.1f,%.1f]\nr (odd,even)=%.2f \nI=%.2f b/sp p= %.3f',UNITS.unit_uid(i_u),  UNITS.brain_area{i_u,1}, UNITS.hemisphere{i_u}, UNITS.cell_type{i_u}, time_window_start(num), time_window_end(num), stability, SI, pvalue));
    ...
    title(sprintf('r (odd,even)=%.1f \nI=%.2f b/sp p= %.3f',stability, SI, pvalue));
...
%     xlabel(vname);
    ...
    xlabel(vname);
    ylabel('FR (Hz)');
    title(sprintf('t wind=[%.1f,%.1f] \nr (odd,even)=%.2f \nI=%.2f b/sp p= %.3f', time_window_start(num), time_window_end(num),stability, SI, pvalue));
    ...
    key_time_dynamics.unit_uid=TUNING{num}.X.unit_uid(i_u);
        key_time_dynamics.tuning_param_name=tuning_param_name{num};
        key_time_dynamics.smooth_flag=key_1D.smooth_flag;
            subplot(3,3,9)

    si_t=fetchn(ANL.UnitTongue1DTuningML*EPHYS.Unit  & key_time_dynamics,'tongue_tuning_1d_si', 'ORDER BY time_window_start');
        r=fetchn(ANL.UnitTongue1DTuningML*EPHYS.Unit  & key_time_dynamics,'stability_odd_even_corr_r', 'ORDER BY time_window_start');
    t_v=fetchn(ANL.UnitTongue1DTuningML*EPHYS.Unit  & key_time_dynamics,'time_window_start', 'ORDER BY time_window_start');
...
x=x{1}';
y=(cell2mat(fetchn( ANL.TongueMLdecoder&(ANL.SessionPosition& key),'ml_performance_right_at_t')));
y=y-y(:,1)
 y=mean(y)
plot(x,y)
...
x=(fetchn( ANL.TongueSVMbinarydecoder & key,'performance_left_at_t'));
x=x{1}';
y=(cell2mat(fetchn( ANL.TongueSVMbinarydecoder& key,'time_vector')));
y=y-y(:,1)
 y=mean(y)
plot(x,y,'-')
