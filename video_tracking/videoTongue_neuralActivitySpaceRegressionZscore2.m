function videoTongue_neuralActivitySpaceRegressionZscore2()
close all;
key.lick_direction='right';

dir_root = 'Z:\users\Arseny\Projects\SensoryInput\SiProbeRecording\';
dir_save_figure = [dir_root 'Results\video_tracking\analysis\regression_session3\' key.lick_direction '\'];
...
time_idx_2plot = (time >=round(tpoint,4) & time<round(tpoint,4) + 0.2);
...
rel_behav= EXP.TrialID & ((EXP.BehaviorTrial * EXP.SessionID * EXP.SessionTraining *ANL.SessionPosition * EXP.TrialName  * ANL.TrialTypeGraphic * ANL.Video1stLickTrialZscoreAllLR) & k  & 'early_lick="no early"' & k_s & ANL.IncludeSession-ANL.ExcludeSession);
    ...
    rel_Proj1 = ((ANL.RegressionProjTrialNormalizedSignif2 &k_proj & rel_behav)  )*EXP.TrialID*EXP.TrialName;
    rel_trials_Proj1 = EXP.TrialID & ((ANL.RegressionProjTrialNormalizedSignif2 &k_proj & rel_behav)  )*EXP.TrialName;
    rel_trials_Proj1=rel_trials_Proj1.proj('trial_uid->trial_uid2');

        
    num=2;
    k_proj.tuning_param_name=tuning_param_name{num};
    k_proj.regression_time_start=regression_time_start(num);
    rel_Proj2 = ((ANL.RegressionProjTrialNormalizedSignif2 &k_proj  & rel_behav)  )*EXP.TrialID*EXP.TrialName;
    rel_trials_Proj2 =EXP.TrialID &  ((ANL.RegressionProjTrialNormalizedSignif2 &k_proj  & rel_behav)  )*EXP.TrialName;
    rel_trials_Proj2=rel_trials_Proj2.proj('trial_uid->trial_uid3');

    
    rel_trials_common=(rel_trials_Proj1&rel_trials_Proj2);
    
    rel_behav=rel_behav&rel_trials_common;
    rel_Proj1=rel_Proj1&rel_trials_common;
    rel_Proj2=rel_Proj2&rel_trials_common;

    if rel_trials_common.count==0
        continue
    end
    ...
    num=1;
    proj_trial=cell2mat(fetchn(rel_Proj1,'proj_trial', 'ORDER BY trial_uid'));
    P(num).endpoint=nanmean(proj_trial(idx_v,time_idx_2plot),2);
    %     hist(endpoint)
    
    num=2;
     proj_trial=cell2mat(fetchn(rel_Proj2,'proj_trial', 'ORDER BY trial_uid'));
    P(num).endpoint=nanmean(proj_trial(idx_v,time_idx_2plot),2);
    ...
    num_units_projected1=fetchn(rel_Proj1,'num_units_projected', 'ORDER BY trial_uid');
    include_proj_idx1=num_units_projected1>minimal_num_units_proj_trial;
   
        num_units_projected2=fetchn(rel_Proj2,'num_units_projected', 'ORDER BY trial_uid');
    include_proj_idx2=num_units_projected2>minimal_num_units_proj_trial;
    
    include_proj_idx=include_proj_idx1& include_proj_idx2;
    ...
    [N,Xedges,Yedges,binX,binY] = histcounts2(P(1).endpoint, P(2).endpoint,linspace(0,1,8),linspace(0,1,8));
