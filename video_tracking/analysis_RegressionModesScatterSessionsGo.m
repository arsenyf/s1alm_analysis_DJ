function analysis_RegressionModesScatterSessionsGo()
close all;
...
dir_save_figure = [dir_root 'Results\video_tracking\analysis\RegressionModes_scatter_sessionsGo\' 't' num2str(key.regression_time_start) '\'];
...
rel_P=(ANL.RegressionProjTrialGo *ANL.SessionPosition *EXP.SessionTraining);
...
for i_p=1:1:numel(tuning_param_name_1D)
    for  i_d=1:1:numel(lick_direction_name)
        ...
        for i_s=1:1:numel(sessions)
            ...
            subplot(6,5,i_s);
            [R2_LinearRegression(i_s),R2_LogisticRegression(i_s)]=fn_plot_regression_scatter_sessionGo (rel_P,  key, tuning_param_label{i_p});
        end
